...
function iE_init(knoten,kanten,M,kk,von,nach)
    mE = kk["RefKante_Infos"]; 
    von_mE = mE["VonNach"][1]; nach_mE = mE["VonNach"][2]
    Params = MakeParam(kk)
    kante = iE_kante(Param=Params, KUL=knoten[von], KUR=knoten[nach], KGL=knoten[von_mE], KGR=knoten[nach_mE], Z=kk)
    push!(kanten, kante)
    append!(M, kante.M)
end

function mE_init(knoten,kanten,i,kk,von,nach)
    nothing
end
=#