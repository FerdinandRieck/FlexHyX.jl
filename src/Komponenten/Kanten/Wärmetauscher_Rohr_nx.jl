Base.@kwdef mutable struct mWTaRnx_Param
    L = 1.0
    nx = 1
    dx = L/max(nx,1/2)
    D = 0.1
...
#-- Rohrränder ---------------------------------
Base.@kwdef mutable struct y_mWTaRnx_randL
    mL::Number = 0.0
    eL::Number = 0.0
end

Base.@kwdef mutable struct mWTaRnx_kante_randL <: Wasser_Kante
    #-- default Parameter
    Param::mWTaRnx_Param
...
Base.@kwdef mutable struct y_mWTaRnx_randR
    mR::Number = 0.0
    eR::Number = 0.0
end

Base.@kwdef mutable struct mWTaRnx_kante_randR <: Wasser_Kante
    #-- default Parameter
    Param::mWTaRnx_Param
...
#-- Rohrmitte ----------------------------------
Base.@kwdef mutable struct y_mWTaRnx_mitte
    P::Number = 1.0 #???Warum diese Anfangswerte in MATLAB???
    _m::Number = 0.0
    T::Number = 3.0
end

Base.@kwdef mutable struct mWTaRnx_kante_mitte <: Wasser_Kante
    #-- default Parameter
    Param::mWTaRnx_Param
...
function Kante!(dy,k,kante::mWTaRnx_kante_randL,t)
    #-- Parameter
...
function Kante!(dy,k,kante::mWTaRnx_kante_randR,t)
    #-- Parameter
    (; dx,Arho,A,D,cv_H2O,mu,K,lamW,phi,g) = kante.Param
    #--
...
function Kante!(dy,k,kante::mWTaRnx_kante_mitte,t)
    #-- Parameter
