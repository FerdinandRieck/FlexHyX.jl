Base.@kwdef mutable struct mWTRK2_Param
    nx = 1
    L = 1.0
    dx = L/max(nx,1/2)
    rho0 = 1000.0
...
Base.@kwdef mutable struct y_mWTRK2
    m::Number = 0.0
    eL::Number = 0.0
    eR::Number = 0.0
    T
...
Base.@kwdef mutable struct mWTRK2_kante <: Wasser_Kante
    #-- default Parameter
    Param::mWTRK2_Param
    ...
    #-- Zustandsvariablen
    y = y_mWTRK2(T = f(Vector(1:2:2*Param.nx), KL.y.T, KR.y.T, Param.nx))
...
function Kante!(dy,k,kante::mWTRK2_kante,t)
    #-- Parameter
    (; nx,dx,leit,Arho,cv_H2O,D,lamW,kA,WENO,fluxTL,fluxTR,rho0) = kante.Param
    #--
